...
def train_forecasting_models(df: pd.DataFrame) -> dict:
    ...
    print(f"\n📊 Training on data from {df['city'].nunique()} cities")
    ...
    for hours in horizons:
        ...
        try:
            ...
            metrics = forecaster.train(df_encoded, target_col='pm25', test_size=0.2)
...
def fetch_real_data_for_all_cities(days: int = 90) -> pd.DataFrame:
    ...
    for city_name, coords in EGYPTIAN_CITIES.items():
        ...
        try:
            city_df = fetcher.fetch_training_dataset(
                city=city_name,
                days=days
            )
            ...
            if not city_df.empty:
                ...
                if 'latitude' not in city_df.columns:
                    city_df['latitude'] = coords['lat']
                if 'longitude' not in city_df.columns:
                    city_df['longitude'] = coords['lon']
                
                all_data.append(city_df)
                print(f"   ✓ Retrieved {len(city_df)} samples")
            else:
    ...
    if all_data:
        ...
        print(f"\n✓ Total samples from all cities: {len(combined_df)}")
        print(f"✓ Cities with data: {combined_df['city'].nunique()}")
        return combined_df
    else:
